...
@section('breadcrumb2') 
	@lang('backend/contents.category.title.add')
@stop

{{-- Content --}}
@section('content')

	{{ Form::open(['route'=>'newCategory', 'POST', 'class'=>'form-horizontal', 'autocomplete'=>'off']) }}
	{{ Form::token() }}
	    <fieldset>
	        <div class="control-group {{ $errors->has('name') ? 'error' : '' }}">
	        	{{ Form::label('name', Lang::get('backend/contents.category.form.name'), ['class'=>'control-label']) }}
	        	<div class="controls">
		            {{ Form::text('name', NULL, ['class'=>'span5']) }}
	        ...
	        <div class="control-group">
	        	<div class="controls">
			        {{ Form::submit(Lang::get('backend/general.button.create'), ['class' => 'btn btn-primary']) }}
